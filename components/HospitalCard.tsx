...
export const HospitalCard: React.FC<HospitalCardProps> = ({ hospital, onSelect }) => {
  return (
    <div className="bg-card rounded-xl shadow-md overflow-hidden transform hover:-translate-y-1 transition-transform duration-300 group border border-border hover:shadow-lg">
      ...
      <div className="p-5">
        <p className="text-foreground font-semibold mb-3">Key Services:</p>
        <div className="flex flex-wrap gap-2 mb-4">
          {hospital.services.slice(0, 3).map(service => (
            <span key={service} className="bg-blue-100 text-blue-800 text-xs font-semibold px-2.5 py-1 rounded-full dark:bg-blue-900/50 dark:text-blue-300">{service}</span>
          ))}
