...
const Section: React.FC<{ title: string; children: React.ReactNode; className?: string }> = ({ title, children, className = '' }) => (
    <div className={`mb-8 ${className}`}>
        <h3 className="text-sm font-bold text-muted-foreground uppercase tracking-wider mb-4 border-b border-border pb-2">{title}</h3>
        {children}
    </div>
);
...
const InfoItem: React.FC<{ icon: React.ReactNode; label: string; value: string | React.ReactNode; }> = ({ icon, label, value }) => (
    <div className="flex items-start text-sm mb-5">
        <div className="text-accent w-5 h-5 mr-4 mt-0.5 flex-shrink-0">{icon}</div>
        <div>
...
export const DoctorProfileModal: React.FC<DoctorProfileModalProps> = ({ doctor, onClose, onBook }) => {
  ...
  const HospitalIcon = () => <svg className="w-5 h-5 text-muted-foreground" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg>;
    
  return (
    <div className="fixed inset-0 bg-black/75 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
      <div className="bg-card rounded-2xl shadow-2xl w-full max-w-4xl relative overflow-hidden max-h-[90vh] flex flex-col md:flex-row">
        <button onClick={onClose} className="absolute top-4 right-4 text-muted-foreground bg-background/50 hover:bg-secondary rounded-full p-1 z-20">
          <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>
        </button>
        
        {/* Left Sidebar */}
        <div className="w-full md:w-1/3 bg-secondary/50 p-8 border-b md:border-b-0 md:border-r border-border flex-shrink-0">
          <div className="text-center">
              <img 
                  src={doctor.imageUrl} 
                  alt={doctor.name} 
                  className="w-32 h-32 rounded-full object-cover border-4 border-background shadow-lg mx-auto mb-4"
              />
              <h2 className="text-2xl font-bold text-foreground">{doctor.name}</h2>
              <p className="text-accent font-semibold">{specialty}</p>
          </div>
          <hr className="my-6 border-border" />
          <div className="space-y-2">
              <InfoItem icon={PhoneIcon} label="Contact" value={<a href={`tel:${doctor.contact}`} className="text-accent hover:underline">{doctor.contact}</a>} />
        ...
        {/* Main Content */}
        <div className="w-full md:w-2/3 p-8 overflow-y-auto bg-background">
          ...
          <Section title="Work Experience">
              ...
              <ul className="list-disc list-inside space-y-2 text-muted-foreground pl-4 text-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8">
              ...
              <Section title="Working Hours">
                  <p className="text-foreground font-semibold text-sm">{doctor.workingHours}</p>
              </Section>
          </div>
          ...
          <div className="pt-4 mt-auto">
              <button 
                  ...
                  className="w-full bg-accent text-accent-foreground font-bold py-3 px-4 rounded-lg hover:bg-opacity-90 transition-all duration-300 shadow-lg hover:shadow-xl focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-ring"
              >
                  Book Appointment
              </button>
          </div>
