        ...
        <div className="bg-gray-50 min-h-screen pt-28">
            <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
                <header className="mb-10">
                    ...
                    <p className="mt-2 text-lg text-gray-600">Here's a summary of your health journey.</p>
                </header>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-start">
                    {/* Main Content: Health Timeline */}
                    <div className="md:col-span-2">
                        <h2 className="text-2xl font-bold text-gray-800 mb-6">Your Health Timeline</h2>
                        <div className="relative pl-6 border-l-2 border-gray-200">
                            {healthHistory.map((event, index) => (
                                <div key={event.id} className="mb-10 relative">
                                    ...
                                    <div className="bg-white p-5 rounded-lg shadow-md border border-gray-100 ml-4">
                                        <time className="text-sm font-semibold text-gray-500 mb-1 block">
                                            {new Date(event.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric', timeZone: 'UTC' })}
                                        </time>
                                        <h3 className="text-lg font-bold text-gray-900">{event.title}</h3>
                                        {event.doctor && (
                                            <p className="text-sm font-medium text-blue-600 my-1">with {event.doctor.name} ({event.doctor.specialty})</p>
                                        )}
                    ...
                    <div className="md:col-span-1 space-y-6 sticky top-28">
                         <div className="bg-white p-6 rounded-lg shadow-md border border-gray-200">
                            ...
                            <div className="space-y-3">
                                ...
                                <button onClick={onNavigateToAppointments} className="w-full bg-gray-100 text-gray-700 font-bold py-2 px-4 rounded-lg hover:bg-gray-200 transition-colors">
                                    View All Appointments
                                </button>
                                <button className="w-full bg-gray-100 text-gray-700 font-bold py-2 px-4 rounded-lg hover:bg-gray-200 transition-colors">
                                    Upload a Document
                                </button>
                            </div>
                        </div>
