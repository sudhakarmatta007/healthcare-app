...
const NavButton: React.FC<{ icon: React.ReactNode; title: string; onClick: () => void; }> = ({ icon, title, onClick }) => (
    <button
        onClick={onClick}
        className="group flex flex-col items-center justify-center w-40 h-40 sm:w-48 sm:h-48 bg-white/5 backdrop-blur-lg border border-white/10 rounded-2xl text-white transform hover:-translate-y-2 transition-all duration-300 hover:bg-white/10 hover:shadow-xl"
    >
        <div className="w-16 h-16 sm:w-20 sm:h-20 mb-3 flex items-center justify-center bg-white/5 rounded-full group-hover:bg-white/10 transition-colors text-gold-300">
            {icon}
        </div>
        <span className="text-lg sm:text-xl font-bold tracking-wide">{title}</span>
    </button>
);
