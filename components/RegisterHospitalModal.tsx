...
export const RegisterHospitalModal: React.FC<RegisterHospitalModalProps> = ({ onClose }) => {
    ...
    if (submitted) {
        return (
            <div className="fixed inset-0 bg-black/75 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
                <div className="bg-card rounded-2xl shadow-2xl w-full max-w-md relative overflow-hidden p-8 text-center">
                    ...
                    <button 
                        onClick={onClose}
                        className="w-full bg-accent text-accent-foreground font-bold py-3 px-4 rounded-lg hover:bg-opacity-90 transition-colors"
                    >
                        Done
                    </button>
                </div>
    ...
    const inputClasses = "mt-1 w-full px-4 py-2 border rounded-lg focus:ring-ring focus:border-ring bg-background text-foreground";
    const errorBorder = "border-destructive";
    const normalBorder = "border-border";

    return (
        <div className="fixed inset-0 bg-black/75 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
            <div className="bg-card rounded-2xl shadow-2xl w-full max-w-lg relative overflow-hidden max-h-[90vh]">
                <button onClick={onClose} className="absolute top-4 right-4 text-muted-foreground hover:text-foreground z-10">
                 ...
                 <div className="p-8">
                     <h2 className="text-2xl font-bold text-center text-foreground mb-2">Register Hospital</h2>
                     <p className="text-center text-sm text-muted-foreground mb-8">Add your hospital to our growing network.</p>
                     <form onSubmit={handleSubmit} className="space-y-4 overflow-y-auto max-h-[70vh] pr-2">
                        <div>
                            <label className="text-sm font-medium text-muted-foreground" htmlFor="name">Hospital Name</label>
                        ...
                        <div>
                            <label className="text-sm font-medium text-muted-foreground" htmlFor="address">Address / Location</label>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label className="text-sm font-medium text-muted-foreground" htmlFor="contact">Contact Number</label>
                        ...
                        <div>
                            <label className="text-sm font-medium text-muted-foreground" htmlFor="specialties">Available Specialties</label>
                            ...
                            {errors.specialties && <p className="text-xs text-destructive mt-1">{errors.specialties}</p>}
                        </div>
                        <div>
                            <label className="text-sm font-medium text-muted-foreground">Upload Hospital Image</label>
                            <label htmlFor="hospital-image-upload" className={`mt-1 flex justify-center w-full px-6 pt-5 pb-6 border-2 border-dashed rounded-lg cursor-pointer hover:border-accent ${errors.image ? errorBorder : normalBorder}`}>
                                <div className="space-y-1 text-center">
                                    <svg className="mx-auto h-12 w-12 text-gray-400" stroke="currentColor" fill="none" viewBox="0 0 48 48" aria-hidden="true"><path d="M28 8H12a4 4 0 00-4 4v20m32-12v8m0 0v8a4 4 0 01-4 4H12a4 4 0 01-4-4v-4m32-4l-3.172-3.172a4 4 0 00-5.656 0L28 28M8 32l9.172-9.172a4 4 0 015.656 0L28 28m0 0l4 4m4-24h8m-4-4v8" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
                                    <div className="flex text-sm text-muted-foreground">
                                        <span className="relative bg-card rounded-md font-medium text-accent hover:opacity-80 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-ring">
                         ...
                         <div className="pt-4">
                            <button 
                                ...
                                className="w-full bg-accent text-accent-foreground font-bold py-3 px-4 rounded-lg hover:bg-opacity-90 transition-colors shadow-lg disabled:bg-muted disabled:text-muted-foreground disabled:cursor-not-allowed"
                            >
